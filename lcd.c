...
void lcd_ram_draw_char(uint16_t x, uint16_t y, char c, uint16_t color, uint32_t *ram) {
    uint8_t i = 0;
    uint8_t j = 0;
    uint8_t data = 0;
    uint16_t *r = (uint16_t *)ram;
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        {
            if (data & 0x80)
                r[(y+i)*LCD_X_MAX+(j+x+1)] = color;
            data <<= 1;
            if (data & 0x80)
                r[(y+i)*LCD_X_MAX+(j+x)] = color;
            data <<= 1;
        }
    }
}
